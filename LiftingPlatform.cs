...
public class LiftingPlatform : MonoBehaviour
{
    ...
    void MovePlatform()
    {
        ...
        if (movingRight && rb.position.y >= topPoint.position.y)
        {
            movingRight = false;
        }

        if (!movingRight && rb.position.y <= buttomPoint.position.y)
        {
            movingRight = true;
        }
    }
